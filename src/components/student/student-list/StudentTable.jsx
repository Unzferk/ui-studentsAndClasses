...
const StudentTable = () => {
  ...
  const searcher = (data) => {
    const dataFiltered = data.filter((student) =>
      searchKeys.some((key) => student[key].toLowerCase().includes(search))
    );
    return dataFiltered;
  };
    ...
    <div className='container container-student__table pt-3 pb-2 pl-2 pr-2'>
      ...
      <div className='table-students-overflow'>
        <table className='table table-sm table-hover col-8 table-overflow'>
          ...
          <tbody>
            {students &&
              searcher(students).map((stud) => {
                return (
                  <tr key={stud.studentId}>
                    <td className='table-students-font-size'>
                      {stud.studentId}
                    </td>
                    <td className='table-students-font-size'>
                      {stud.firstName}
